...
class DataIngestion:
    ...
    #     csv_path = os.path.join(current_dir, 'data', r'C:\MY_Folder\MY_Courses\1.GEN_AI_LIVE_Classes\ipynb_files\CUSTOMER_SUPPORT_SYSTEM\data\flipkart_product_review.csv')
    ...
    def _get_csv_path(self):
        csv_path = r'C:\MY_Folder\MY_Courses\1.GEN_AI_LIVE_Classes\ipynb_files\CUSTOMER_SUPPORT_SYSTEM\data\flipkart_product_review.csv'
        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"CSV file not found at: {csv_path}")
        return csv_path
    ...
    def run_pipeline(self):
        ...
        for res in results:
            print(res.page_content.encode('utf-8', errors='replace').decode('utf-8'))
            # print(f"Content: {res.page_content}\nMetadata: {res.metadata}\n")
            with open("search_output.txt", "a", encoding="utf-8") as f:
                f.write(f"Content: {res.page_content}\nMetadata: {res.metadata}\n\n")
